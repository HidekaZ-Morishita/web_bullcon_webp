function initFooter() {
    ...
    fetch('/html/_footer.html?v=' + Date.now(), fetchOptions)
        .then(response => { if (!response.ok) { throw new Error('Failed to load footer: ' + response.statusText); } return response.text(); })
        .then(data => {
        ...
        .catch(error => {
            ...
            const footerPlaceholder = document.getElementById('footer-placeholder');
            if (footerPlaceholder) {
                footerPlaceholder.innerHTML = '<p>フッターの読み込みに失敗しました。</p>';
            }
        });
}
