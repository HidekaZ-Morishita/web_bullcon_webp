...
.map(year => { const yearMatch = year.match(/\d{4}/); return { value: yearMatch ? yearMatch[0] : year, text: year }; }); populateOptions(document.getElementById('year-select'), [{ value: 'unknown', text: '【年式不明の方はこちら】' }, ...processedYears], '年'); if (selectedYear) { document.getElementById('year-select').value = selectedYear; } if (selectedYear) { const productCodeGroup = document.createElement('div'); productCodeGroup.className = 'form-group'; productCodeGroup.innerHTML = `<label>モニター型番</label>`; if (selectedYear === 'unknown') { const inputValue = selectedProductCode !== null ? selectedProductCode : ''; productCodeGroup.innerHTML += `
...
.map(item => item.product_code); populateOptions(selectElement, filteredCodes, '品番を選択してください'); if (selectedProductCode) { selectElement.value = selectedProductCode; } } } } } } } } export async function renderForm(containerId, state) { const { searchMode = 'car', selectedProduct, reversePartNumber = '' } = state; const formContainer = document.getElementById(containerId); if (!formContainer) { return; } formContainer.innerHTML = ''; const tabsContainer = document.createElement('div'); tabsContainer.className = 'search-mode-tabs'; tabsContainer.innerHTML = `
<div class="search-mode-tab ${searchMode === 'car' ? 'active' : ''}" data-mode="car">製品・車種から探す</div>
<div class="search-mode-tab ${searchMode === 'reverse' ? 'active' : ''}" data-mode="reverse">品番から逆引きする</div>
